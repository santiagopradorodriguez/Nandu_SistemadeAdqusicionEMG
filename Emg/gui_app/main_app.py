...
try:
    from analisis_por_track_integrado import procesar_wavs_promedio
except ImportError:
...
class ReaperStyleHub(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("EMG Analytics Studio (DAW Edition)")
        ...
        self.setWindowTitle("Ñandu LSD - Estación Biomédica (EMG)")
    ...
    def _create_main_toolbar(self):
        ...
        utils = [
            ("Metrónomo", "metronomo_visual.py"),
            ("Editar Medición", "editor_mediciones.py"),
            ("Extraer Datos ML", "extractor_de_datos_procesados.py"),
            ("Graficador", "plotter_calibrado.py"),
            ("Análisis Correlación", "correlaciondeseñales.py")
        ]
    ...
    def _setup_tabs(self):
        ...
        <div style='padding: 20px;'>
            ...
            <ul>
                <li><b>Framework Moderno:</b> Refactorización total desde Tkinter a PyQt6/PySide6, garantizando fluidez y aceleración gráfica por hardware.</li>
                <li><b>Visores Nativos (CSV Viewer):</b> Visor de datos 100% integrado con filtros interactivos, downsampling LTTB y zoom dinámico. Filtro Pasa-Banda activo por defecto.</li>
                <li><b>Countdown Interactivo:</b> La ventana de Adquisición cuenta ahora con un semáforo interactivo que sincroniza visual y sonoramente el inicio de tus grabaciones musculares.</li>
                <li><b>Estética Cyberpunk:</b> Renovación completa del Metrónomo Visual y de los paneles de análisis.</li>
            </ul>
            ...
            <p>👉 <a href='{file_url}' style='color:#00aaff; font-size: 14px;'><b>Abrir justificacion_matematica.md</b></a></p>
        </div>
        """
    ...
    def _on_explorer_selection_changed(self):
        ...
        if n > 0:
            if canales_totales:
                ...
                tab_proc.lbl_canales_warn.setText("No se detectaron subcarpetas de canal en estas mediciones.")
                tab_proc.lyt_canales_procesar.addWidget(tab_proc.lbl_canales_warn)
        else:
            tab_proc.lbl_canales_warn.setText("Selecciona mediciones en el Gestor para ver los canales...")
            tab_proc.lyt_canales_procesar.addWidget(tab_proc.lbl_canales_warn)
