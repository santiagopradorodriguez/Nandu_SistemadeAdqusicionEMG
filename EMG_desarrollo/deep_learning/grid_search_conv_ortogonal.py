...
# Modulo: Grid Search Epico de Autoencoders Convolucionales 1D Ortogonales
# Evaluacion: Exactitud GMM Lucas + Transferencia Directa a Secuencia Continua P5
...
out_dir_default = os.path.join(
    project_root,
    "EMG_desarrollo/resultados/grid_search_conv_ortogonal"
)
...
# ==============================================================================
# 1. ARQUITECTURA PARAMETRIZABLE CON BUFFER DE IDENTIDADES PRECALCULADO
# ==============================================================================
class ParametricConvOrthogonalAE(nn.Module):
    def __init__(self, in_channels=3, time_pts=20, conv_channels=(8, 16), kernel_size=3, latent_dim=2, act_name='tanh'):
        super().__init__()
...
# ==============================================================================
# 3. EVALUACION DE UNA CONFIGURACION
# ==============================================================================
def evaluar_configuracion(cfg, X_lucas_t, y_lucas, X_p5_t, y_p5, device, seed=100, epochs=350):
...
# ==============================================================================
# 4. GRAFICO Y GUARDADO DE MODELO CAMPEON
# ==============================================================================
def graficar_campeon_record(cfg, res, y_lucas, y_p5, p_png):
    fig, axes = plt.subplots(1, 3, figsize=(18, 5.5))
    
    # Subplot 1: Espacio Latente Lucas
    ax0 = axes[0]
    Z_l = res['Z_lucas']
    for v in vocales:
        mask = (y_lucas == v)
        ax0.scatter(Z_l[mask, 0], Z_l[mask, 1], c=colores_oficiales[v], label=f"Vocal /{v.lower()}/", alpha=0.7, edgecolors='none', s=25)
    ax0.axhline(0, color='gray', linestyle='--', linewidth=0.8, alpha=0.5)
    ax0.axvline(0, color='gray', linestyle='--', linewidth=0.8, alpha=0.5)
    ax0.set_title(f"Espacio Latente Lucas: Exactitud {res['acc_lucas']:.1f}%", fontsize=11, fontweight='bold')
    ax0.set_xlabel("Coordenada Latente Z1")
    ax0.set_ylabel("Coordenada Latente Z2")
    ax0.legend(frameon=True, fontsize=9, loc='upper right')
    ax0.grid(True, linestyle=':', alpha=0.6)
    
    # Subplot 2: Espacio Latente P5
    ax1 = axes[1]
    Z_p = res['Z_p5']
    y_p5_idx = np.array([vocal_to_idx[v] for v in y_p5])
    ...
    for v in vocales:
        ...
        ax1.scatter(Z_p[mask, 0], Z_p[mask, 1], c=colores_oficiales[v], marker='o', label=f"/{v.lower()}/ Correcto", alpha=0.85, s=35)
        mask_err = (y_p5 == v) & (~aciertos)
        if np.any(mask_err):
            ax1.scatter(Z_p[mask_err, 0], Z_p[mask_err, 1], c=colores_oficiales[v], marker='x', label=f"/{v.lower()}/ Error", alpha=0.9, s=45, linewidths=1.5)
            
    num_correct = int(np.sum(aciertos))
    ax1.axhline(0, color='gray', linestyle='--', linewidth=0.8, alpha=0.5)
    ax1.axvline(0, color='gray', linestyle='--', linewidth=0.8, alpha=0.5)
    ax1.set_title(f"Secuencia Continua P5: {res['acc_p5']:.1f}% - {num_correct}/123 Pulsos", fontsize=11, fontweight='bold')
    ax1.set_xlabel("Coordenada Latente Z1")
    ax1.set_ylabel("Coordenada Latente Z2")
...
def guardar_campeon(cfg, res, y_lucas, y_p5, out_dir, records_count, prev_record):
    os.makedirs(out_dir, exist_ok=True)
    
    p_pt = os.path.join(out_dir, "modelo_campeon_conv_ortogonal.pt")
    p_json = os.path.join(out_dir, "config_campeon.json")
    p_p5_csv = os.path.join(out_dir, "proyecciones_campeon_p5.csv")
    p_lucas_csv = os.path.join(out_dir, "proyecciones_campeon_lucas.csv")
    p_png = os.path.join(out_dir, "grafico_campeon_record.png")
    ...
    # Grafico comparativo
    graficar_campeon_record(cfg, res, y_lucas, y_p5, p_png)

def anunciar_record(cfg, res, total_comb, current_idx, records_count, prev_record, out_dir):
    num_correct_p5 = int(round(res['acc_p5'] * 123 / 100.0))
    sep = "=" * 80
    print("\n" + sep)
    print(f"RECORD HISTORICO SUPERADO: {res['harmonic_acc']:.2f}% | Record previo: {prev_record:.2f}%")
    print(f"Combinacion [{current_idx}/{total_comb}] | Hito #{records_count}")
    print(f"Arquitectura: Kernel={cfg['kernel_size']} | Canales={cfg['channels']} | Activacion={cfg['act']}")
    ...
    print(f"  - Lucas (Entrenamiento): {res['acc_lucas']:.2f}%")
    print(f"  - Secuencia Continua P5: {res['acc_p5']:.2f}% ({num_correct_p5}/123 pulsos)")
    print(f"  - Desglose P5: /a/={res['acc_A']:.1f}% | /e/={res['acc_E']:.1f}% | /i/={res['acc_I']:.1f}% | /o/={res['acc_O']:.1f}% | /u/={res['acc_U']:.1f}% | Piso: {res['min_vocal_p5']:.1f}%")
    print(f"Artefactos del campeon exportados en:")
    print(f"  -> {os.path.join(out_dir, 'modelo_campeon_conv_ortogonal.pt')}")
    print(f"  -> {os.path.join(out_dir, 'config_campeon.json')}")
    print(f"  -> {os.path.join(out_dir, 'proyecciones_campeon_p5.csv')}")
    print(f"  -> {os.path.join(out_dir, 'grafico_campeon_record.png')}")
    print(sep + "\n")
    sys.stdout.flush()
    ...
    try:
        msg_voz = f"Nuevo record detectado. Media armonica {res['harmonic_acc']:.1f} por ciento."
...
# ==============================================================================
def main():
    parser = argparse.ArgumentParser(description="Grid Search Masivo para Autoencoder Convolucional Ortogonal")
    ...
    parser.add_argument('--baseline-record', type=float, default=83.00, help="Record previo de media armonica a batir (default: 83.00)")
    ...
    csv_results = os.path.join(out_dir_default, f"resultados_grid_search_{args.mode}.csv")
    ...
    print("=" * 80)
    print("INICIANDO GRID SEARCH EPICO DE AUTOENCODERS CONVOLUCIONALES ORTOGONALES")
    ...
    for idx, cfg in enumerate(combinations):
        ...
        res = evaluar_configuracion(cfg, X_lucas_t, y_lucas, X_p5_t, y_p5, device=device, seed=args.seed, epochs=args.epochs)
        ...
        if res['harmonic_acc'] > best_harmonic and res['min_vocal_p5'] >= 50.0:
            ...
            records_count += 1
            best_cfg = row
            guardar_campeon(cfg, res, y_lucas, y_p5, out_dir_default, records_count, prev_record)
            anunciar_record(cfg, res, total_comb, current_idx, records_count, prev_record, out_dir_default)
        ...
        print(f"[{current_idx:4d}/{total_comb:4d}] ({((current_idx)/total_comb)*100:5.1f}%) | "
              ...
              f"Lucas: {res['acc_lucas']:5.1f}% | P5: {res['acc_p5']:5.1f}% | Armonica: {res['harmonic_acc']:5.1f}% | "
              f"ETA: {eta_s/60:4.1f}m")

    print("\n" + "=" * 80)
    print("GRID SEARCH EPICO FINALIZADO")
