...
# Descripción: Módulo de análisis y proyecciones PCA con visualizaciones 2D/3D.
# ==============================================================================
...
if os.path.basename(script_dir_abs) == "deep_learning":
    deep_learning_dir = script_dir_abs
else:
...
def plot_analisis_errores_3d(X, Y, Tomas, title, output_path, variance_ratios=None, algoritmo="K-Means", is_umap=False, ocultar_leyenda=False, **kwargs):
    ...
    print("\n--- DETALLE DE MEDICIONES MAL CLASIFICADAS (UMAP 3D) ---")
    errores_encontrados = False
    legend_elements = []
...
def extraer_y_filtrar(mediciones, base_dir, params, aplicar_trevisan, modo_alineacion, pre_pct, post_pct, canales_features, ignorar_ventana_cero=False, cache_canales_data=None, verbose=True, aplicar_correccion_intersesion=False):
    X, Y, Tomas, SNRs = extraer_features_concatenadas(
        base_dir, mediciones, 
        alpha_ruido=params.get('alpha_ruido', 0.5), 
    ...
    snr_thresh = params.get('snr_threshold', 0.5)
    outlier_contam = params.get('outlier_contamination', 0.05)
    ...
    for vocal in np.unique(Y):
        ...
        valid_snr_mask = SNRs_vocal >= snr_thresh
        ...
        if len(X_vocal_snr) > 5 and outlier_contam > 0:
            iso = IsolationForest(contamination=outlier_contam, random_state=42)
...
def ejecutar_procesamiento(
    ...
    def plot_confusion_matrix_heatmap(df_cm, title, filepath):
        ...
        # Lineas tipo booktabs (toprule, midrule, bottomrule)
    ...
    for p in plots_to_open:
        if os.path.exists(p):
            try:
                # Usar xdg-open en Linux para simular el visor "interactivo" de autoencoders
...
class GeneradorPCAGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("Generador Dataset PCA")
        ...
        self.var_proc_pca_2d = tk.BooleanVar(value=True)
    ...
    def iniciar_procesamiento(self):
        ...
        try:
            ...
            params_3d = {
                ...
                "gate_ratio_ruido": float(self.ent_gate_3d.get()),
            ...
            params_umap = {
                ...
                "gate_ratio_ruido": float(self.ent_gate_umap.get()),
