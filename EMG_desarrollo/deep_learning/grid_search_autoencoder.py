...
def run_grid_search(
    ...
    Por defecto recorre 36 combinaciones:
      - 6 dimensiones latentes: [2, 3, 4, 8, 16, 32]
      - 3 tamaños de kernel: [3, 5, 7]
      - 2 factores de balance alpha: [0.3, 0.7]
    """
    _set_seed(42)
    
    if latent_dims is None:
        latent_dims = [2, 3, 4, 8, 16, 32]
    ...
    for l_dim in latent_dims:
        for k_size in kernel_sizes:
            for alpha in alphas:
                ...
                combo_name = f"Lat{l_dim}D_K{k_size}_A{alpha}"
                if verbose:
                    print(f"\n[{combo_idx:2d}/{total_combinaciones}] Evaluando: LatentDim={l_dim:2d}, Kernel={k_size}, Alpha={alpha:.1f} ...", end=" ", flush=True)
    ...
    # Crear columna combinada (Kernel, Alpha) para el eje X
    df_plot = df_resultados.copy()
    df_plot["config_cnn"] = df_plot.apply(lambda r: f"K={int(r['kernel_size'])} | a={r['alpha']:.1f}", axis=1)
    ...
    axes[0].set_xlabel("Configuración CNN (Kernel y Alpha)")
    ...
    axes[1].set_xlabel("Configuración CNN (Kernel y Alpha)")
