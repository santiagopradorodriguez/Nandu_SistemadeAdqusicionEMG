...
def _detect_maxima_and_extract(env_recortada,
    ...
    if n_pulsos_manual is not None and n_pulsos_manual > 0:
        ...
        print("--- ERROR: No se encontró un 'pulse_count' válido en metadata.json. Omitiendo archivo. ---")
        return [], []
...
def procesar_wavs_promedio(
    ...
    for filename in archivos:
        ...
        start_sample_noise, env_noise, sigma_est, umbral, noise_rms_from_noise_window = _estimate_noise_window(
            signal_recortada, samplerate, noise_seconds, smooth_ms, factor_umbral, tipo_envolvente)
        if start_sample_noise <= 0:
            start_sample_noise = 0
        ...
        maxima_per_cut, _ = _detect_maxima_and_extract(
            ...
            search_threshold_dinamico, n_pulsos_manual=n_pulsos_manual, excluded_windows=initial_excluded)
        ...
        for i in range(n_pulsos_total):
            ...
            if noise_level > (umbral * 10.0):
                print(f"[Aviso] Ruido interpulso muy alto en pulso {i+1} ({noise_level:.3f}). Usando ruido base ({umbral:.3f}).")
                noise_level = umbral
        ...
        if picos_ventana:
            ...
            print(f"[Umbral activación] Calculado con P{int(activation_percentile)} de {len(picos_ventana)} ventanas buenas: {activation_threshold:.4f}")
        else:
...
class ProcessingOptionsDialog(tk.Toplevel):
    ...
    def procesar(self, interactivo=True):
        ...
        for med_name, ch_list in meas_to_channels.items():
            if interactivo:
                excl_path = os.path.join(self.BASE_DIR, med_name, "excluded_windows.json")
                if os.path.exists(excl_path):
                    with open(excl_path, 'r') as f:
                        excl_data = json.load(f)
                        final_excl = excl_data.get("excluded_windows", [])
                else:
        ...
        if sweep_meas_data:
            try:
                ...
                plt.tight_layout()
                boxplot_sep_path = os.path.join(self.BASE_DIR, "separabilidad_picos_boxplot.png")
        ...
        if sweep_canal and sweep_meas_data:
            ...
            if feasible_selected:
                ...
                bp = ax.boxplot([th0_vals, th1_vals, th2_vals], labels=['Canal 0', 'Canal 1', 'Canal 2'], patch_artist=True)
                ...
                ax.legend()
                plt.tight_layout()
                boxplot_path = os.path.join(self.BASE_DIR, "umbrales_boxplot.png")
            ...
            plt.tight_layout()
            purity_path = os.path.join(self.BASE_DIR, "porcentajes_vocales.png")
            ...
            sens_path = os.path.join(self.BASE_DIR, "sensibilidad_umbrales.png")
        ...
        for med_name, canales_data in datos_para_plot_combinado.items():
            ...
            out = os.path.join(self.BASE_DIR, med_name, "senal_corregida_combinada.png")
        ...
        if global_patterns:
            ...
            try:
                ...
                cube_path = os.path.join(self.BASE_DIR, "espacio_motor_3d.png")
