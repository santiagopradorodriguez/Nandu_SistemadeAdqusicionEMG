...
def main(res_dir=None):
    if res_dir is None:
        ...
        candidate_dirs = [
            os.path.join(script_dir, 'resultados_pca_umap'),
            os.path.join(os.path.dirname(script_dir), 'deep_learning', 'pca_umap_clustering', 'resultados_pca_umap'),
            os.path.join(os.getcwd(), 'resultados_pca_umap')
        ]
        for d in candidate_dirs:
            if os.path.exists(os.path.join(d, 'metricas.txt')):
                res_dir = d
                break
        if res_dir is None:
            res_dir = candidate_dirs[0]
    ...
    if not os.path.exists(metricas_path):
        print(f"[Aviso] No se encontró el archivo de métricas en: {metricas_path}")
        return
    ...
    print(f"Gráficos procesados para: {res_dir}")
