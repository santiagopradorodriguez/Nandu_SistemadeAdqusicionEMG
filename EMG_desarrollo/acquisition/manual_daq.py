...
def simulador_thread(chunk_samples, sample_rate, num_canales, data_queue, stop_event, test_freq=50, tipo_prueba="Senoidal"):
    ...
    if "Archivo" in tipo_prueba:
        try:
            ...
            test_dir = os.path.join(os.path.dirname(script_dir), "base_de_datos_electrodos", "2026-05-22", "_E4_1_NONE _Sujeto2")
...
def generar_grafico_grabacion(datos_completos, sample_rate, output_dir, num_canales, canales_daq, base_name="photo"):
    ...
    # Crear figura y lienzo explícitamente sin invocar pyplot (thread-safe)
...
class RealTimePlotter(QtWidgets.QWidget):
    ...
    def _setup_ui_config_panel(self):
        ...
        fs_def = str(int(adq_conf.get("frecuencia_muestreo", 2000.0)))
        if fs_def not in ["2000", "6000", "10000", "20000", "44100", "50000"]:
            self.cmb_sample_rate.addItem(fs_def)
        self.cmb_sample_rate.setCurrentText(fs_def)
        ...
        adq_conf = config_mgr.get("adquisicion") or {}
        nidaq_channels = adq_conf.get("nidaq_channels", ["Dev1/ai0", "Dev1/ai1", "Dev1/ai2", "Dev1/ai3"])
        
        for ch_str in nidaq_channels:
            chk = QtWidgets.QCheckBox(ch_str.split("/")[-1])
    ...
    def setup_gui_for_channels(self):
        ...
        for i in range(self.NUM_CANALES):
            ...
            label.setStyleSheet(f"color: {color}; font-size: 14px; font-weight: bold; background-color: transparent; border: none;")
        ...
        self.CURRENT_FFT_LEN = self.SPECTROGRAM_FFT_LEN
    ...
    def on_filter_settings_changed(self):
        ...
        if is_filter_enabled and self.is_acquiring and SCIPY_DISPONIBLE:
            ...
            if low_cut >= high_cut:
                ...
                # Para N canales, la forma correcta del array de estados es (n_sections, N_CANALES, 2).
    ...
    def actualizar_plot(self):
        ...
        try:
            ...
            if getattr(self, '_init_filter_state', False):
                initial_vals = processed_data[:, 0].reshape(1, self.NUM_CANALES, 1)
                if self.notch_zi is not None:
                    self.notch_zi = self.notch_zi * initial_vals
                if self.filter_zi is not None:
                    self.filter_zi = self.filter_zi * initial_vals
                self._init_filter_state = False

            # Aplicar filtros (si están habilitados) al bloque calibrado
            # --- NUEVO: Aplicar filtro Notch primero (Vectorizado) ---
            if self.chk_notch_enable.isChecked() and self.notch_sos is not None and self.notch_zi is not None:
                notch_filtered_data, self.notch_zi = signal.sosfilt(self.notch_sos, processed_data, axis=-1, zi=self.notch_zi)
                processed_data = notch_filtered_data

            # --- Aplicar filtro Pasa-Banda después (Vectorizado) ---
            if self.chk_filter_enable.isChecked() and self.filter_sos is not None and self.filter_zi is not None:
                bandpass_filtered_data, self.filter_zi = signal.sosfilt(self.filter_sos, processed_data, axis=-1, zi=self.filter_zi)
                processed_data = bandpass_filtered_data
            
            # 5. Actualizar el buffer de ploteo (Copia in-place para evitar lag de recolección de basura)
            if total_muestras_leidas >= self.PLOT_SAMPLES:
                self.plot_buffer_datos[:, :] = processed_data[:, -self.PLOT_SAMPLES:]
            else:
                self.plot_buffer_datos[:, :-total_muestras_leidas] = self.plot_buffer_datos[:, total_muestras_leidas:]
                ...
                if np.any(mask_superan):
                    ...
                    if getattr(self, 'is_recording', False) and getattr(self, 'noise_calculated', False):
                        if self.noise_levels[i] > 0:
                            snr = avg_peak_val / self.noise_levels[i]
                            texto += f" | SNR(Avg): {snr:.1f}"
                else:
    ...
    def actualizar_espectrograma(self, new_data):
        ...
        try:
            # Obtener los datos del canal seleccionado desde el buffer principal (ventana móvil)
            canal_idx = self.spectrogram_channel_index
            if canal_idx < self.NUM_CANALES and self.plot_buffer_datos.shape[1] >= self.CURRENT_FFT_LEN:
                # Tomar los últimos N puntos del buffer de visualización
                segmento = self.plot_buffer_datos[canal_idx, -self.CURRENT_FFT_LEN:]
                
                # Aplicar ventana de Hann
                ventana = np.hanning(self.CURRENT_FFT_LEN)
                segmento_ventaneado = segmento * ventana
                
                # Calcular la FFT real
                espectro = np.abs(np.fft.rfft(segmento_ventaneado))
                
                # Escala logarítmica para mejor visualización
                espectro_log = np.log10(espectro + 1e-12)
                
                # Desplazar el buffer del espectrograma hacia la izquierda
                self.spectrogram_buffer = np.roll(self.spectrogram_buffer, -1, axis=0)
                # Añadir la nueva columna al final
                self.spectrogram_buffer[-1, :] = espectro_log
    ...
    def closeEvent(self, event):
        ...
        if hasattr(self, 'timer_plot'):
            self.timer_plot.stop() # Detiene el timer de la GUI
